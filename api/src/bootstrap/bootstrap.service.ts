...
export class BootstrapService {
  ...
  private async ensureDefaultPipeline(tenantId: string) {
    ...
    // Ensure B2C pipeline exists. In this variant, B2C is hotel guest lifecycle and marketing follow-up.
    let b2c = find('Guest Lifecycle') || find('B2C');
    if (!b2c) {
      const created = await this.prisma.pipeline.create({
        data: {
          tenantId,
    ...
    } else if (b2c.name === 'B2C') {
      await this.prisma.pipeline.update({
        where: { id: b2c.id },
        data: { name: 'Guest Lifecycle' },
      });
      b2c = { ...b2c, name: 'Guest Lifecycle' };
    }

    if (b2c) {
      await this.ensureHospitalityB2CStages(tenantId, b2c.id);
    }
  ...
  private async ensureHospitalityB2CStages(tenantId: string, pipelineId: string) {
    ...
    const legacyRename = new Map([
      ['Lead', 'Newsletter capture'],
      ['Qualified', 'Segmented guest'],
      ['Offer', 'Campaign planned'],
      ['Checkout', 'Stay follow-up'],
      ['Won', 'Return booked'],
      ['Lost', 'No response'],
    ]);

    for (const stage of existing) {
      const nextName = legacyRename.get(stage.name);
      if (nextName) {
        await this.prisma.stage.update({ where: { id: stage.id }, data: { name: nextName } });
      }
    }
    ...
    const byName = new Map(refreshed.map((stage) => [stage.name, stage]));

    for (const stage of desiredStages) {
      const current = byName.get(stage.name);
      if (!current) {
        await this.prisma.stage.create({ data: { ...stage, tenantId, pipelineId } });
        continue;
      }

      if (
        current.position !== stage.position ||
